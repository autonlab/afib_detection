...
train_data_groups = {0:[],1:[],2:[]}
for pat in train_data:
    g0 = [tup for tup in pat if tup[1]<1*60*60 and tup[2]]
    g1 = [tup for tup in pat if tup[1]>=1*60*60 and tup[1]<2*60*60 and tup[2]]
    g2 = [tup for tup in pat if tup[1]>=2*60*60]
    train_data_groups[0].append(g0)
    train_data_groups[1].append(g1)
    train_data_groups[2].append(g2)

# convert to data loaders
for j in range(len(train_data_groups)):
    for i in range(len(train_data_groups[0])):
        train_data_groups[j][i] = DataLoader(train_data_groups[j][i], batch_size = 32, shuffle=True)

train_dataloader = DataLoader(sum(train_data,[]), batch_size=64, shuffle=True)
print("Train class balance:")
print( ClassBalance(train_dataloader) )

test_dataloader = DataLoader(sum([PrepareData(data[pat],LABEL_CODE) for pat in test_patients],[]), batch_size=64, shuffle=True)
...
for epoch in range(5000):
    ...
    for x,y,d in train_dataloader:
        ...
        optimizer.zero_grad()
        a = model(x)
        loss = model.RiskTrajectoryLoss(y,d,a,H)
        loss.backward()
        optimizer.step()
        avg_loss += loss.item()
        c += 1.
